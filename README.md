...
- [Publish on Render (private website)](#publish-on-render-private-website)
...
| `REQUIRE_PASSWORD` | `false` | Website only: `true` refuses to start without an `APP_PASSWORD` of 12+ characters |
...
3. Fill in the values it asks for:
   ...
   `SESSION_SECRET` is generated by Render for you.
4. Click **Apply**. The first build takes a few minutes. Render deploys the
   `main` branch.
...
- Every page's data, and the `/docs` pages, need the password. Only the sign-in
  page and the `/health` check (used by Render, no personal data) are public.
- After 5 wrong passwords from one address, or 20 in total, sign-in waits
...
backend\
  ...
  auth.py               the website's password sign-in and browser security headers
...
Dockerfile, render.yaml publishing as a private website on Render
