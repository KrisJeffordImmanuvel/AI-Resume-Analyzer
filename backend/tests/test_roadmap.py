...
def result():
    return {
        "matched": [
            {"skill": "Python", "category": "Programming Languages", "priority": "required", "credit": 1.0,
             "match_type": "exact", "resume_evidence": [EV], "jd_evidence": [EV]},
            {"skill": "CI/CD", "category": "DevOps & Infrastructure", "priority": "required", "credit": 0.5,
             "match_type": "ai_inferred", "resume_evidence": [{**EV, "quote": "used GitHub Actions"}],
             "jd_evidence": [EV]},
        ],
        "missing": [
...
def test_fallback_roadmap_orders_gaps_and_uses_templates():
    ...
    assert [(i["skill"], i["kind"]) for i in roadmap["items"]] == [
        ("Kubernetes", "learn"), ("CI/CD", "strengthen"), ("Terraform", "learn"),
    ]
...
def test_ai_roadmap_uses_ai_steps_but_never_ai_links():
    provider = FakeProvider({"items": [
        {"skill": "kubernetes", "steps": ["Learn pods", "Deploy with kubectl", " "],
