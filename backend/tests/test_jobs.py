...
RESUMES = {
    name: (SAMPLES / name).read_bytes()
    for name in ("sample_resume.txt", "sample_resume_devops.txt", "sample_resume_frontend.txt")
}
...
def test_job_skills_come_from_the_engine_with_priorities():
    ...
    assert [s["priority"] for s in skills] == sorted(
        [s["priority"] for s in skills], key=["required", "standard", "preferred"].index
    )
...
def test_candidates_are_ranked_and_compared_side_by_side(make_client):
    ...
    assert by_file["sample_resume.txt"]["cells"]["Kubernetes"] == {
        "status": "missing",
        "match_type": None,
        "quote": None,
    }
...
def test_bad_files_and_duplicates_do_not_stop_the_batch(make_client):
    with make_client() as client:
        ...
        files = [
            ("resumes", ("sample_resume.txt", RESUMES["sample_resume.txt"])),
            ("resumes", ("old.doc", b"x")),
            ("resumes", ("empty.txt", b"")),
...
def test_limits_and_validation(make_client):
    with make_client() as client:
        ...
        too_many = client.post(
            f"/api/jobs/{job['id']}/candidates", files=[("resumes", (f"r{i}.txt", b"Python")) for i in range(11)]
        )
        no_jd = client.post("/api/jobs", data={"title": "x"})
        bad_jd = client.post("/api/jobs", files={"jd_file": ("jd.pdf", b"%PDF")})
        missing = [
            client.get("/api/jobs/999").status_code,
...
def test_delete_job_removes_it_and_its_candidates_permanently(make_client):
    ...
    with make_client() as client:
        ...
        seeker = client.post(
            "/api/analyses", files={"resume": ("cv.txt", b"Python")}, data={"jd_text": "Python"}
        ).json()
