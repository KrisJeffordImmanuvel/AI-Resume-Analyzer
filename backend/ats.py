...
def parse_preview(resume_text: str) -> dict:
    ...
    if words < 150:
        issues.append(
            "Very little text was extracted. If the resume looks full, parts may be images or text boxes "
    ...
    for section in ("Experience", "Education", "Skills"):
        if section not in headings:
            issues.append(
                f"No “{section}” heading was recognised. Standard headings help software "
                "split your resume into sections."
            )
    table_lines = sum(1 for line in lines if line.count(" | ") >= 1)
    if table_lines >= 3:
        issues.append(
            f"{table_lines} lines look like table rows. Some ATS read tables out of order; plain lines are safer."
        )
    return {
        "text": resume_text,
...
def keyword_diff(resume_text: str, jd_text: str) -> dict:
    ...
    return {
        ...
        "summary": {
            ...
            "different_wording": sum(
                1 for r in rows if r["kind"] == "skill" and r["skill_in_resume"] and not r["exact_in_resume"]
            ),
        },
    }
    ...
    checks = [
        {
            ...
            "detail": (
                f"Top of the resume mentions: {', '.join(headline_hits)}"
                if headline_hits
        ...
        },
        {
            ...
            "detail": (
                ", ".join(top_skills) if top_skills else "No skills from the job description in the first lines."
            ),
        },
        {
